...
@section('page-header')
				...
				<div class="breadcrumb-header justify-content-between">
					<div class="my-auto">
						<div class="d-flex">
							<h4 class="content-title mb-0 my-auto">الفواتير</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ قائمة</span>
						</div>
					</div>
...
@section('content')
			 ...
			 @endif
				...
				<div class="row">
					...
					@can('تصدير EXCEL')
										...
										<a class="modal-effect btn btn-outline-primary btn-block"  href="{{url('exportinvoices')}}"> تصدير فاتورةالى اكسل</a>
					@endcan
												...
												<div class="dropdown">
													...
													<div  class="dropdown-menu tx-13">
														...
														<a class="dropdown-item" 
                                                                        ...
                                                                        <i class="las fa-money-bill"></i>&nbsp;&nbsp;تعديل حالة الدفع
														</a>
														<a href="#model_move" class="dropdown-item modal-effect " 
                                                        data-invoices-id="{{$invoice->id}}"
                                                                        data-toggle="modal" >
                                                         <i class="tex-warning fas fa-exchange-alt"> </i>نقل الى الارشيف  
														</a>
														<a class="dropdown-item" 
                                                                        href="Print_invoice/{{ $invoice->id }}">
                                                                        <i class="text-success fas fa-print"></i>&nbsp;&nbsp;طباعة فاتورة
														</a>
													</div>
												</div>
												</td>
		...
		<!-- End Modal alert message -->
				 	<!-- Modal alert message ارشفة الفاتورة-->
				...
				<div class="modal-content tx-size-sm">
					<div class="modal-body tx-center pd-y-20 pd-x-20">
						...
						<form action="{{route('invoices.destroy' , 'test')}}" method="post">
						    {{method_field('delete')}}
							{{csrf_field()}}
						<h4 class="tx-success mg-b-20">تحذير هل انت متاكد من نقل الفاتورة الى الارشيف </h4>
						<input type="hidden" name="invoices_id" id="invoices_id" value="">   
...
$('#model_move').on('show.bs.modal',function(event){
    var button = $(event.relatedTarget)
    var id = button.data('invoices-id');
